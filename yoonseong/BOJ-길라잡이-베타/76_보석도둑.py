...
for bag in bags:
    # 보석 정보가 있고, 보석의 무게가 가방의 무게 이하일 때 까지 pop
    while jewelryInfo and jewelryInfo[0][0] <= bag:
        # 보석 가격이 높은게 루트에 위치하도록 -를 곱해서 heap에 push (최대힙)
        heapq.heappush(tmp, -jewelryInfo[0][1])
    ...
    # (가벼운 무게를 들 수 있는 가방이 앞에 있기 때문에 이전에 뽑은 무게들은 다음 가방에서 들 수 있음)
    if tmp:
