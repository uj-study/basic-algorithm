...
def solution(gems):
    ...
    left, right = 0, 0 # left는 보석을 제거, right는 보석을 추가
    gem_dict = {gems[0] : 1} # 첫번째 위치보석 초기값 세팅

    while left < len(gems) and right < len(gems): # 투 포인터가 범위를 벗어나면 종료
    ...
    return [answer[0] + 1, answer[1] + 1] # 시작 인덱스가 1이기 때문에 1증가
